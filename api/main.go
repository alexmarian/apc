...
func main() {
	...
	mux.HandleFunc(fmt.Sprintf("GET /v1/api/associations/{%s}/owners", handlers.AssociationIdPathValue), apiCfg.MiddlewareAssociationResource(handlers.HandleGetAssoctiationOwners(apiCfg)))
	srv := &http.Server{
		Addr:    ":" + port,
		Handler: mux,
	}
